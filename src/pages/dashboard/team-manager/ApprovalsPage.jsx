...
export default function TmApprovalsPage() {
  ...
  // Fetch current user's team_id first
  useEffect(() => {
    if (!user?.id) return
    supabase
      .from('users')
      ...
      .then(({ data }) => {
        ...
        else setLoading(false)  // no team assigned
      })
  }, [user?.id])

  // Fetch applications filtered by this team
  const fetchApps = useCallback(async () => {
    if (!teamId) return
    setLoading(true)
    let q = supabase
      .from('player_applications')
      .select('*, users!player_applications_reviewed_by_fkey(name)')
      .eq('team_id', teamId)
    ...
    <DashboardLayout title="Approvals">
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20 }}>
        <div>
          ...
          <p style={{ fontSize:12, color:'var(--text-muted)' }}>
            Review dan approve/tolak pendaftaran player tim kamu yang masuk via link undangan.
          </p>
        </div>
      ...
      {!teamId && !loading ? (
        <div className="card" style={{ textAlign:'center', padding:'48px 24px' }}>
          <User size={36} style={{ color:'var(--text-dim)', margin:'0 auto 12px' }}/>
          <p style={{ fontSize:13, color:'var(--text-muted)' }}>
            Kamu belum terhubung ke tim manapun. Hubungi Super Admin.
          </p>
        </div>
      ) : loading ? (
