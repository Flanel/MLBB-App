// InviteRegisterPage.jsx — v5
// Nickname field: IGN text + Game User ID + Zone ID (seperti format MLBB)
// Security: honeypot, UUID token validation, sanitize, rate limit
...
import { Eye, EyeOff, XCircle, Loader, AlertTriangle, Mail, Clock, Shield } from 'lucide-react'
...
export default function InviteRegisterPage() {
  ...
  // Player fields
  const [nickname,   setNickname]   = useState('')   // IGN text
  const [gameUserId, setGameUserId] = useState('')   // User ID numerik
  ...
  useEffect(() => {
    if (!token || !isValidToken(token)) { setTokenStatus('invalid'); return }
    async function fetchToken() {
      const { data, error } = await supabase.from('invite_tokens')
        .select('*, teams(name)').eq('token', token).single()
      if (error || !data) { setTokenStatus('invalid'); return }
      if (new Date(data.expires_at) < new Date()) { setTokenStatus('expired'); return }
      if (data.max_uses != null && (data.use_count ?? 0) >= data.max_uses) { setTokenStatus('full'); return }
      setTokenData(data)
  ...
  // Format IGN lengkap: "nickname|gameUserId(zoneId)"
  function buildIGN() {
    if (!gameUserId && !zoneId) return nickname
    if (!zoneId) return `${nickname} | ${gameUserId}`
    return `${nickname} | ${gameUserId} (${zoneId})`
  }
  ...
  async function handleSubmit(e) {
    ...
    if (submitCount.current > 5) { setError('Terlalu banyak percobaan.'); return }
    ...
    const cleanGameId   = (gameUserId || '').replace(/\D/g, '').substring(0, 20)
    const cleanZoneId   = (zoneId || '').replace(/\D/g, '').substring(0, 10)
    const cleanFullName = sanitizeName(fullName)
    const cleanBirth    = sanitizeName(birthPlace)
    const cleanProvince = sanitizeText(province, 50)
    const cleanCity     = sanitizeName(city)
    const cleanStaff    = sanitizeName(staffName)
    const cleanIGN      = sanitizeIGN(buildIGN())
    ...
    if (password.length > 128)       { setError('Password terlalu panjang.'); return }

    if (isPlayer) {
      if (!cleanNickname)      { setError('Nickname / IGN wajib diisi.'); return }
      if (!cleanFullName)      { setError('Nama Lengkap wajib diisi.'); return }
      if (!cleanBirth)         { setError('Kota Lahir wajib diisi.'); return }
    ...
    try {
      ...
      if (signupErr) {
        ...
        if (low.includes('security purposes') || low.includes('only request this after')) { setRateLimitSecs(parseRateLimitSecs(msg)); return }
        if (low.includes('rate limit')) { setError('Server sedang sibuk. Coba lagi.'); return }
        if (low.includes('already registered')) { setError('Email sudah terdaftar.'); return }
        throw new Error(msg)
      }
      const userId = authData.user?.id
      if (!userId) throw new Error('Gagal membuat akun.')
      if (authData.user?.identities?.length === 0) { setError('Email sudah terdaftar.'); return }
      if (!authData.session) throw new Error('Konfigurasi server belum selesai. Hubungi administrator.')

      const isActive = !isManager
      const userInsert = isPlayer
        ? { id:userId, email:cleanEmail, name:cleanNickname, ign:cleanIGN, nickname:cleanNickname,
            game_user_id:cleanGameId||null, zone_id:cleanZoneId||null,
            role:'player', team_id:tokenData.team_id, is_active:isActive,
            province:cleanProvince||null, city:cleanCity||null }
        : { id:userId, email:cleanEmail, name:cleanStaff,
            role:tokenData.role, team_id:tokenData.team_id, is_active:isActive }

      const { error: insertErr } = await supabase.from('users').upsert(userInsert, { onConflict:'id' })
      if (insertErr) throw new Error(`Gagal simpan profil: ${insertErr.message}`)

      if (isPlayer) {
        await supabase.from('player_applications').insert({
          invite_token_id: tokenData.id, user_id:userId, team_id:tokenData.team_id,
          nickname:cleanNickname, ign:cleanIGN,
          game_user_id:cleanGameId||null, zone_id:cleanZoneId||null,
          full_name:cleanFullName, birth_place:cleanBirth, birth_date:birthDate,
          province:cleanProvince, city:cleanCity, esport_type:esportType, status:'approved',
        }).then(()=>{}).catch(()=>{})
      }

      if (isManager) {
        await supabase.from('manager_pending_approvals').insert({
          user_id:userId, team_id:tokenData.team_id, name:cleanStaff, email:cleanEmail, status:'pending',
        }).then(()=>{}).catch(()=>{})
      }

      await supabase.from('invite_tokens').update({
        use_count: (tokenData.use_count??0)+1,
        ...((tokenData.use_count??0)===0?{used_at:new Date().toISOString()}:{}),
      }).eq('id', tokenData.id)

      navigate('/login', { state:{ registered:true, role:tokenData.role, pendingApproval:isManager } })
    } catch (err) {
  ...
  if (['invalid','expired','full'].includes(tokenStatus)) {
    const M = {
      invalid:{ icon:XCircle,       color:'var(--red)',   title:'Link Tidak Valid',  body:'Link ini tidak ditemukan atau sudah tidak berlaku.' },
      expired:{ icon:AlertTriangle, color:'#f59e0b',      title:'Link Kedaluwarsa',  body:'Link ini sudah melewati batas waktu. Minta link baru.' },
      full:   { icon:AlertTriangle, color:'var(--brand)', title:'Link Penuh',        body:'Kuota link sudah penuh. Minta link baru.' },
    }
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="card" style={{ display:'flex',flexDirection:'column',gap:14 }}>
            ...
            <div>
              ...
              <div style={{ position:'relative' }}>
                <input className="form-input" style={{ paddingRight:36 }} type={showPw?'text':'password'}
                  placeholder="Min. 8 karakter" value={password} onChange={e=>setPassword(e.target.value)} minLength={8} maxLength={128} required />
            ...
            {isPlayer && (
              <>
                ...
                {/* Nickname + Game ID + Zone ID — seperti format MLBB */}
                <div>
                  <label className="form-label">Nickname / IGN *</label>
                  <div style={{ display:'flex', gap:8, alignItems:'stretch' }}>
                    {/* Nickname text */}
                    <input className="form-input" style={{ flex:'1 1 0' }}
                      placeholder="Nickname in-game" maxLength={30}
                      value={nickname} onChange={e=>setNickname(e.target.value)} required />

                    {/* Game User ID */}
                    <div style={{ position:'relative', width:110, flexShrink:0 }}>
                      <input className="form-input" style={{ paddingLeft:18, fontFamily:'IBM Plex Mono,monospace', fontSize:12, width:'100%' }}
                        placeholder="User ID" maxLength={20}
                        value={gameUserId} onChange={e=>setGameUserId(e.target.value.replace(/\D/g,''))} />
                      <span style={{ position:'absolute', left:7, top:'50%', transform:'translateY(-50%)', fontSize:11, color:'var(--text-dim)', pointerEvents:'none' }}>|</span>
                    </div>

                    {/* Zone ID */}
                    <div style={{ position:'relative', width:76, flexShrink:0 }}>
                      <input className="form-input" style={{ paddingLeft:10, fontFamily:'IBM Plex Mono,monospace', fontSize:12, width:'100%' }}
                        placeholder="Zone" maxLength={10}
                        value={zoneId} onChange={e=>setZoneId(e.target.value.replace(/\D/g,''))} />
                  ...
                  <p style={{ fontSize:10,color:'var(--text-dim)',marginTop:4 }}>
                    Format: <span style={{ fontFamily:'IBM Plex Mono,monospace', color:'var(--text-muted)' }}>{nickname||'Nickname'} | {gameUserId||'12345678'} ({zoneId||'1234'})</span>
                  </p>
                </div>

                <div>
                  <label className="form-label">Nama Lengkap *</label>
                  <input className="form-input" placeholder="Sesuai KTP" maxLength={100}
                ...
                <div style={{ background:'var(--bg-elevated)',borderRadius:8,padding:'12px 14px',border:'1px solid var(--border-1)' }}>
                  ...
                  <div style={{ display:'flex',flexDirection:'column',gap:10 }}>
                    ...
                    <div>
                      ...
                      <input className="form-input" placeholder="e.g. Jakarta Selatan" maxLength={80}
            ...
            <button type="submit" className="btn btn-primary" disabled={submitting||rateLimitSecs>0}
              style={{ width:'100%',justifyContent:'center',marginTop:4 }}>
              {submitting?'Mendaftarkan...':`Daftar sebagai ${ROLE_LABEL[tokenData?.role]||'Member'}`}
            </button>
            {isManager && <p style={{ fontSize:11,color:'var(--text-dim)',textAlign:'center' }}>Akun Team Manager perlu diverifikasi Super Admin.</p>}
